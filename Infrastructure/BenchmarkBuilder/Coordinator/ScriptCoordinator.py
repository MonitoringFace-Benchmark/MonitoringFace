...
from Infrastructure.constants import PATH_TO_ARCHIVE, FINGERPRINT_DATA, PATH_TO_FOLDER, Signature_File, Policy_File
...
class ScriptCoordinator(Coordinator):
    def __init__(self, path_manager: PathManager, script_contract: ScriptSetupContract, online_experiment_settings: OnlineExperimentContractGeneral):
        ...
        self.policy_dst_path = f"{self.path_to_data_folder}/{Policy_File()}"

        self.policy = Policy_File() if os.path.exists(self.policy_org_path) else None
    ...
    def build(self):
        ...
        if os.path.exists(self.policy_org_path):
            shutil.copy(self.policy_org_path, self.policy_dst_path)
